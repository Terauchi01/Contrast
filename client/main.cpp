...
class ContrastClient {
   public:
    ContrastClient(std::string desired_role, std::string nickname, std::string model)
    ...
    int run() {
        ...
        std::thread input(&ContrastClient::input_loop, this);
        reader.join();
        running_ = false;
        if (input.joinable()) {
            input.join();
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        ContrastClient client(role, name, model);
        return client.run();
    } catch (const std::exception& ex) {
